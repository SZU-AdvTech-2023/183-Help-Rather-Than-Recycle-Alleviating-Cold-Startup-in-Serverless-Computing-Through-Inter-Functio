...
fig, (ax1, ax2) = plt.subplots(1, 2, gridspec_kw=dict(width_ratios=[1, 1]), constrained_layout=True)
# 添加标题
# plt.title("冷启动次数函数占比图", fontproperties='KaiTi', fontsize=20, color="red")  # 添加图名
...
# 构造数据
labels = '1次以内', '2-5次', '大于5次'  # 设置显示标签

# 设置突出显示的标签
explode = [0, 0, 0]

# 设置标签颜色
colors = ['#FFC0CB', '#FFFACD', '#7FFFD4']

# generate fig_7_b
y2 = res["pagurus"].values
for i in range(0, len(y2)):
    if y2[i] <= 1:
        ...
        times[2] += 1
size = [times[0], times[1], times[2]]  # 设置每个标签对应的值
elements = ["1次以内", "2-5次", "大于5次"]
...
plt.xlabel("不同冷启动次数对应的函数占比", fontsize = 14, loc="center")
plt.savefig("azure_result/fig_7.pdf", bbox_inches='tight')
